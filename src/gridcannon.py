...
def initGrid(r,c):
    ...
    for i in range(3):
        for j in range(3):
            ...
            img = Image.open(serialize(grid[index][TOP],IDN) if grid[index] else D)
    ...
    for i in range(5):
        for j in range(3 if i==0 or i==4 else 2):
            # get corresponding image and convert to ImageTk
            img = Image.open(serialize(royals[index][0][TOP],IDN) if royals[index][0] else D)
            img = ImageTk.PhotoImage(img)
            # store reference to image
    ...
    for i in range(2):
        # get corresponding image and convert to ImageTk
        img = Image.open(serialize(ploys[i][TOP],IDN) if ploys[i] else D)
        img = ImageTk.PhotoImage(img)
        # store reference to image
        bottomIMGS[i*3] = img
        # create a button for the image and store
...
def update(gi,ri,bi):
    ...
    for i in gi:
        # get corresponding image and convert to ImageTk
        img = Image.open(serialize(grid[i][TOP],IDN) if grid[i] else D)
        img = ImageTk.PhotoImage(img)
        # store reference to image
        gridIMGS[i] = img
    ...
    for i in ri:
        # get corresponding image and convert to ImageTk
        img = Image.open(serialize(royals[i][0][0],IDN,royals[i][1],True if len(royals[i][0])>1 else False) if royals[i][0] else D)
        img = ImageTk.PhotoImage(img)
        # store reference to image
        royalsIMGS[i] = img
        # configure button to new image
    ...
    for i in bi:
        ...
        if i==1:
            img = Image.open(D if deck.empty() else IDN+"BG.png") # NOTE: card back image set here
        elif i==2:
            img = Image.open(serialize(current,IDN) if current else D)
        else:
            img = Image.open(serialize(ploys[int(i/3)][TOP],IDN) if ploys[int(i/3)] else D)
...
def init(root,menu,callback,info):
    ...
    # determine IDN
    global IDN
    IDN = "assets/Gridcannon/" if os.path.isdir("assets") else "../assets/Gridcannon/"
    # set D using IDN
    global D
    D = IDN+"OLW.png"
    ...
    ref = "assets/Standard/" if os.path.isdir("assets") else "../assets/Standard/"
    # resize the entire deck and store in a dir called Gridcannon
    global PT
    resize(gc.winfo_screenwidth(), gc.winfo_screenheight(), 6, 5, [[1.15,2,"H"],[1.25,2,"V"]], [[PT,1]], True, ref, IDN)
    # initialize grid gui
    initGrid(2,1)
    # place gc frame
